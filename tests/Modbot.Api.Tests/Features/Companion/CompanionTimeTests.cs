...
public class CompanionTimeTests(PostgresFixture db)
{
    [Fact]
    public async Task TheTimeAnswerIsOnlyTheServersTime()
    {
        var ct = TestContext.Current.CancellationToken;
        await using var host = await CompanionApiTestHost.StartAsync(db);

        using var response = await host.Client.GetAsync("/api/v1/companion/time", ct);
