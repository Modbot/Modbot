...
public class OverlayReadTests
{
    ...
    private static async Task WriteAsync(CompanionApiTestHost host, params FactRecord[] facts)
    {
    ...
    private static async Task<T> GetAsync<T>(
        CompanionApiTestHost host, string token, string path, CancellationToken ct)
    {
    ...
    private static Task<HttpResponseMessage> PollAsync(
        CompanionApiTestHost host, string token, CancellationToken ct, int wait = 1)
        => host.Client.SendAsync(host.WithToken(HttpMethod.Get, $"/api/v1/companion/alerts?wait={wait}", token), ct);
    ...
    public async Task AModeratorsLogStopping_EndsTheRoster()
    {
        ...
        Assert.Equal(2, (await GetAsync<InstanceContextDto>(
            host, token, $"/api/v1/companion/context?instanceId={Instance}", ct)).Members.Count);
        ...
        Assert.Empty((await GetAsync<InstanceContextDto>(
            host, token, $"/api/v1/companion/context?instanceId={Instance}", ct)).Members);
    }
    ...
    public async Task AnInstanceNobodyHasReportedIsAnEmptyRosterRatherThanAnError()
    {
        ...
        var roster = await GetAsync<InstanceContextDto>(
            host, token, "/api/v1/companion/context?instanceId=nobody-here", ct);
    ...
    public async Task SomebodyWithNothingOnRecordIsASuccessfulEmptyAnswerRatherThanA404()
    {
        ...
        var summary = await GetAsync<UserSummaryDto>(host, token, "/api/v1/companion/user/usr_nobody", ct);
    ...
    public async Task AProfileSummaryCarriesWhatFitsOnAHeadsetCard()
    {
        ...
        var summary = await GetAsync<UserSummaryDto>(host, token, "/api/v1/companion/user/usr_sub", ct);
    ...
    public async Task TheReadsRefuseAnUnknownToken()
    {
        ...
        foreach (var path in (string[])
            [$"/api/v1/companion/context?instanceId={Instance}", "/api/v1/companion/user/usr_x", "/api/v1/companion/alerts?wait=1"])
        {
    ...
    public async Task PresenceObservedRaisesNothingEvenForAFlaggedUser()
    {
        ...
        var report = host.WithToken(HttpMethod.Post, "/api/v1/companion/events", reporter);
        report.Content = JsonContent.Create(new EventBatchDto(
