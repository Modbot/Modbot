...
public class PairingTokenTests
{
    ...
    public void RoundTripsThroughTheTokenAndTheLink()
    {
        ...
        Assert.StartsWith("modbot-companion://pair?token=", token.ToLink());
    }
    ...
    {
        var token = new PairingToken(new Uri("https://modbot.example/"), "AB12-CD34");
        var link = "MODBOT-COMPANION://PAIR?token=" + Uri.EscapeDataString(token.Encode());
    ...
    [InlineData("modbot-companion://pair?nothing=here")]
    [InlineData("modbot-companion://elsewhere?token=abc")]
    ...
    [Theory]
    [InlineData("modbot-companion://pair?token=x", true)]
    [InlineData("  MODBOT-COMPANION:anything", true)]
