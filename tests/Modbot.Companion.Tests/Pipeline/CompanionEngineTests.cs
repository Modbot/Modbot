...
/// </summary>
public sealed class CompanionEngineTests : IDisposable
{
    private sealed class CapturingTransport : IIngestTransport
    {
        public Dictionary<string, List<CompanionEvent>> ByServer { get; } = new(StringComparer.Ordinal);
    ...
    private async Task DrainAsync(CompanionEngine engine)
    {
    ...
    public async Task OneLogFeedsEveryServerWithoutBeingReadTwice()
    {
        ...
        var engine = new CompanionEngine(observer, _clock, [cats, dogs]);
    ...
    /// </summary>
    private static async Task<(List<string> Cloud, List<CompanionEvent> Server)> RunWithCloudAsync(string directory, bool paired)
    {
        ...
        var engine = new CompanionEngine(new PresenceObserver(new VRChatLogTail(logs), clock), clock, connections, backup: backup);
    ...
    public async Task EachServerGetsItsOwnIdempotencyKeysForTheSameEvent()
    {
        ...
        var catIds = _transport.ByServer["cats"].Select(e => e.CompanionEventId);
        var dogIds = _transport.ByServer["dogs"].Select(e => e.CompanionEventId);
    ...
    public async Task TheClockIsResyncedPeriodicallyRatherThanPerBatch()
    {
        ...
        var cats = Connection("cats", LogFixture.GroupId);
        var engine = new CompanionEngine(observer, _clock, [cats], probe);

        await engine.TickAsync(TestContext.Current.CancellationToken);
        await engine.TickAsync(TestContext.Current.CancellationToken);
        Assert.Equal(1, probe.Calls);

        _clock.Advance(CompanionEngine.ClockCheckInterval + TimeSpan.FromMinutes(1));
        await engine.TickAsync(TestContext.Current.CancellationToken);
    ...
    public async Task AQuietTickDoesNothing()
    {
        var observer = Observer();
        CopyFixture();
        var engine = new CompanionEngine(observer, _clock, [Connection("cats", LogFixture.GroupId)]);
        await engine.TickAsync(TestContext.Current.CancellationToken);
