...
public class NotifyOverlaySettingsTests : IDisposable
{
    ...
    public void AFileWithNoNotifyOverlayGetsTheDefaults()
    {
        ...
        Assert.Equal(NotificationSettings.Default, settings);
    ...
    {
        var chosen = new NotificationSettings(
            On: false,
    ...
    public void WritingItLeavesEveryOtherFieldAlone()
    {
        ...
        Assert.True(CompanionSettings.SaveNotifyOverlay(Path_, NotificationSettings.Default with { Spot = ScreenSpot.TopLeft }));
    ...
    public void AHandEditedFileCannotPutThePanelWhereNobodyCanFindIt()
    {
        ...
        Assert.Equal(NotificationSettings.Default.Spot, settings.Spot);
        Assert.Equal(NotificationSettings.MaxDistance, settings.Distance);
        Assert.Equal(NotificationSettings.MinWidth, settings.Width);
        Assert.Equal(NotificationSettings.MinOpacity, settings.Opacity);
        Assert.Equal(NotificationSettings.MaxSeconds, settings.Seconds);
        Assert.Equal(NotificationSettings.MaxFine, settings.Across);
    }
    ...
    public void ATypoInTheFileIsNotOverwritten()
    {
        ...
        Assert.False(CompanionSettings.SaveNotifyOverlay(Path_, NotificationSettings.Default));
    ...
    {
        var placement = (NotificationSettings.Default with { Spot = spot }).ToPlacement();
    ...
    public void MovingThePanelFurtherAwayKeepsItInTheSamePlaceInTheView()
    {
        ...
        var near = (NotificationSettings.Default with { Distance = 1f }).ToPlacement();
        var far = (NotificationSettings.Default with { Distance = 2f }).ToPlacement();
    ...
    {
        var plain = NotificationSettings.Default.ToPlacement();
        var nudged = (NotificationSettings.Default with { Across = 0.1f, Down = 0.05f }).ToPlacement();
    ...
    {
        Assert.Equal(NotificationSettings.Default, NotificationSettings.FromJson(new JsonObject()));
        Assert.Equal(NotificationSettings.Default, NotificationSettings.FromJson(null));
    }
}
