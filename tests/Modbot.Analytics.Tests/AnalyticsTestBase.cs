...
/// </summary>
/// <remarks>
/// Shared by the daily totals and retention suites: both need a database nobody else is writing to --
...
{
    /// <summary>
    /// Fixed, like the fact tests': daily totals are keyed by day, and a suite that quietly depends on
    ...
    protected DailyTotalsJob NewJob(ModbotContext context) => new(context, Clock);

    /// <summary>
    /// Writes facts through the real writer, creating whatever partitions they need first.
    /// </summary>
    /// <remarks>
    /// Through <see cref="FactWriter"/> and not straight into the table, so that the daily totals are
    ...
    /// <summary>Every daily total row, ordered, as comparable text.</summary>
    ...
    protected async Task<decimal?> ValueAsync(DateOnly day, string metric, string dimension = "")
    {
        ...
        var row = await context.DailyTotals
            .AsNoTracking()
