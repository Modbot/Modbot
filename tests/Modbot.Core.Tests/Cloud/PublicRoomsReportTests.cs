...
/// <summary>
/// What goes in the public rooms report, and what must never go in it.
/// </summary>
/// <remarks>
/// The rules here are the ones the privacy policy states: only rooms anyone can join, never a room
...
public class PublicRoomsReportTests
{
    private static readonly DateTimeOffset Evening = new(2026, 9, 16, 21, 0, 0, TimeSpan.Zero);

    private readonly PostgresFixture _db;

    public PublicRoomsReportTests(PostgresFixture db) => _db = db;
    ...
    /// <summary>A group with one room of each access type open, plus a closed public one.</summary>
        ...
        string? groupName = "Night Owls")
    {
        ...
        settings.SharePublicRooms = shared;
        ...
        // A public room that has already closed, and a public room belonging to somebody else.
        context.VRChatInstances.Add(new VRChatInstance
        {
            Id = Guid.NewGuid(),
    ...
    private static PublicRoomsReportBuilder Builder(ModbotContext context, bool cloudDisabled = false) =>
    ...
    public async Task TheSettingIsOnForANewModbot()
    {
        ...
        Assert.True(settings.SharePublicRooms);
    }

    [Fact]
    public async Task OnlyRoomsAnyoneCanJoinAreReported()
    {
        ...
        Assert.Contains("groupAccessType(public)", room.Location, StringComparison.Ordinal);
    }

    [Fact]
    public async Task AMembersOnlyRoomIsNeverReported()
    {
        ...
        Assert.NotNull(report);
        Assert.DoesNotContain(report.Rooms, r => r.Location.Contains("groupAccessType(members)", StringComparison.Ordinal));
        Assert.DoesNotContain(report.Rooms, r => r.Location.Contains("groupAccessType(plus)", StringComparison.Ordinal));
    }

    [Fact]
    public async Task ARoomThatHasClosedIsNotReported()
    {
        var (_, context) = await SeedAsync();
        await using var _ = context;
        ...
        Assert.NotNull(report);
        Assert.DoesNotContain(report.Rooms, r => r.Location.Contains(":closed~", StringComparison.Ordinal));
    }

    [Fact]
    public async Task ARoomThatClosesTakesItselfOutOfTheNextReport()
    {
        var (group, context) = await SeedAsync();
        await using var _ = context;
        ...
        Assert.Empty(after.Rooms);
    }

    [Fact]
    public async Task AnotherGroupsRoomIsNotReported()
    {
        var (_, context) = await SeedAsync();
        await using var _ = context;
        ...
        Assert.NotNull(report);
        Assert.DoesNotContain(report.Rooms, r => r.Location.Contains("grp_somebody_else", StringComparison.Ordinal));
    }
    ...
    public async Task TheReportCarriesTheWorldTheLinkAndTheGroupsPictures()
    {
        ...
        Assert.Equal("https://pictures.test/world.png", room.WorldImageUrl);
        Assert.Equal("us", room.Region);
        Assert.Equal(Evening, room.OpenedAt);
        Assert.StartsWith("https://vrchat.com/home/launch?worldId=", room.JoinLink, StringComparison.Ordinal);
    }

    /// <summary>
    /// The report has no field for a head count, so nothing serialised from it can carry one. The
    /// rooms in the fixture each have 27 people in them, and the report is checked field by field
    ...
    public async Task NobodyIsCounted()
    {
        ...
        Assert.Equal(["groupId", "groupName", "groupIconUrl", "groupBannerUrl", "rooms"], top);

        var room = json.RootElement.GetProperty("rooms").EnumerateArray().Single();
        var fields = room.EnumerateObject().Select(p => p.Name).ToList();
        Assert.Equal(
    ...
    public void OnlyHttpsPicturesLeaveTheServer(string? given, string? expected) =>
        Assert.Equal(expected, PublicRoomsReportBuilder.Picture(given));
}
