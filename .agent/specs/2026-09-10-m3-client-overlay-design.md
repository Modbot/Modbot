...
*Superseded for the tester phase by §9.4: what ships today is a Velopack per-user installer. The
reasoning below is the end state a signed general release should return to.*
...
### 9.4 What ships today: Velopack, per-user, from GitHub Releases

*Recorded September 2026, when the client first became something a tester could download rather
than `dotnet run`. This narrows §8.3, §8.4 and §9.2 for the tester phase; it does not replace
them as the end state.*

**The problem.** §8.4 and §9 describe a signed MSI in Program Files served from a central update
host with publisher-pinned Authenticode verification. None of that exists yet, and all of it
waits on a code-signing identity the project does not have (§8.2). Meanwhile the only way to run
the client was from a source tree with the SDK, which is not a thing testers do, and §9.3 is
already true: every parser fix needs to reach machines.

**The decisions, and where they differ from the sections above.**

| | The spec above | What ships today | Why |
|---|---|---|---|
| Installer | WixSharp MSI, Program Files (§8.4) | **Velopack** `Setup.exe`, per-user, `%LOCALAPPDATA%\Modbot` | No administrator prompt for an unsigned build to fail at; delta updates; GitHub Releases as the feed with nothing to host. Add/Remove Programs entry, Start Menu entry and a real uninstaller are all present, which is the part of §8.3 that actually maps to a heuristic. |
| Layout | Folder, never single-file (§8.3) | **Folder**, self-contained win-x64, ReadyToRun, trimming off | Unchanged. Single-file was tested against and rejected for the reason §8.3 gives; the overlay's `openvr_api.dll` and Skia's native libraries are ordinary files beside the exe. |
| Version | Generated by the build (foundation §2.7.2) | `-p:ModbotRelease=YYYY.M.PATCH` → `ModbotVersion.Release`, `AssemblyVersion`, the installer and the feed | Velopack takes the calendar version as-is (SemVer), so the MSI `ProductVersion` mapping in foundation §2.7.2 is not needed yet. It will be if the MSI returns. |
| Update feed | Central host, static manifest (§9.1) | **GitHub Releases** of the repository the release workflow publishes to, baked into the build | One place; nothing to run. Still central, still the project's, still the supply-chain surface §9.2 names. |
| Payload verification | Authenticode + pinned publisher (§9.2) | **SHA-256 from the feed, over HTTPS** (Velopack's own check). Not Authenticode. | There is no signing identity to pin. This is the gap a certificate closes first; until then the feed's transport security and GitHub's account security are what stands between a release and a client. |
| Consent | Tells and asks (§9.2) | **Tells, and never restarts.** Downloaded in the background; one line in the window; installed only at the next start, which the moderator chooses by quitting and reopening. `"checkForUpdates": false` in `settings.json` turns the check off. | A restart underneath a running session is a presence gap that cannot be filled in later, so the client never forces one. Asking a question in a tray app that nobody is looking at is a dialog that gets dismissed; the line stays until acted on. |
| Deltas | Full payloads only (§9.2) | **Deltas on** (Velopack default), hash-verified against the full package after applying | The size argument in §9.2 was made before a self-contained publish came in at ~120 MB. A tester on a slow connection would otherwise download that every week during a parser-chasing month. |
| Channels | Stable and beta | **One channel** | Nobody to be on beta yet. |

**What did not change.** `%APPDATA%\Modbot` — pairings, queues, the sent record, the logs — is the
client's and is never touched by install, update or uninstall, so an update keeps every pairing
and a reinstall picks up where the old one left off. The single-instance mutex is won *before* a
downloaded update is installed, so the second copy a browser link starts can never swap the files
under the running one. The source guard's ban on process launching still covers every file the
client ships; Velopack, which starts its own `Update.exe`, is reachable from exactly one file and
that file carries the disclosure.

**What still needs a certificate.** Signed `Setup.exe`, exe and dlls (the workflow already takes
the certificate as a secret and skips cleanly without it); SmartScreen reputation; the
publisher-pinned payload check of §9.2; and only then the Program Files MSI, if a general release
turns out to want one.
