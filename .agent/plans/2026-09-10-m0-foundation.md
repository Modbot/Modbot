...
| 7 | Rollups: `modbot_rollup_daily`, `RollupJob`, recompute | Property test: recomputed == incremental | Generic `(day, metric, dimension)` shape so new metrics need no migration (spec §5.4). Include the §5.2.1 counted-only path. |
