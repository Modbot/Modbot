...
dark one. The mascot head sits on the left with the wordmark under it, the section's name is pushed
to the far right on the wordmark's line, and a violet rule runs under both.
...
The rule has squared ends, no rounding, and is violet the whole way across. Every banner carries
the same wordmark at the same size, whatever the name on the right is, so a row of them lines up.
