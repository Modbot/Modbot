...
/// window that sits over VRChat on a monitor, written whole the same way the voice is.
/// And <c>clips</c>
/// (<c>{ "on": false, "minutes": 3, "folder": "…", "keepGigabytes": 5 }</c>), keeping the last few
/// minutes of the screen while VRChat runs — off unless somebody turns it on — written whole the same way.</para>
