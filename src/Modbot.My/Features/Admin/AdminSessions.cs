...
public sealed class AdminSessions(RootApiKey rootKey, TimeProvider time)
{
    ...
    private const string Purpose = "my.modbot.co admin session";
    ...
    public async Task<(string Token, DateTimeOffset ExpiresAt)> StartAsync(MyContext db, CancellationToken ct)
    {
    ...
    public async Task<bool> IsValidAsync(MyContext db, string? token, CancellationToken ct)
    {
        ArgumentNullException.ThrowIfNull(db);
    ...
    public async Task EndAsync(MyContext db, string? token, CancellationToken ct)
    {
        ArgumentNullException.ThrowIfNull(db);
