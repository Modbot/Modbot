...
public sealed partial class MainWindow
{
    ...
    private NotifyOverlaySettings NotifySettings => _snapshot.NotifyOverlayOrNone.SettingsOrDefault;
    ...
    {
        var notifications = _snapshot.NotifyOverlayOrNone;
        ...
        reset.Click += (_, _) => _actions.SetNotifyOverlay(NotifyOverlaySettings.Default with { On = NotifySettings.On });
    ...
    private void WireNotifyControls()
    {
        ...
        _notifyAcross = NotifySlider(-NotifyOverlaySettings.MaxFine, NotifyOverlaySettings.MaxFine, 0.02, (s, v) => s with { Across = (float)v });
        _notifyDown = NotifySlider(-NotifyOverlaySettings.MaxFine, NotifyOverlaySettings.MaxFine, 0.02, (s, v) => s with { Down = (float)v });
        _notifyDistance = NotifySlider(NotifyOverlaySettings.MinDistance, NotifyOverlaySettings.MaxDistance, 0.05, (s, v) => s with { Distance = (float)v });
        _notifyWidth = NotifySlider(NotifyOverlaySettings.MinWidth, NotifyOverlaySettings.MaxWidth, 0.05, (s, v) => s with { Width = (float)v });
        _notifyOpacity = NotifySlider(NotifyOverlaySettings.MinOpacity, 1, 0.05, (s, v) => s with { Opacity = (float)v });
        _notifySeconds = NotifySlider(NotifyOverlaySettings.MinSeconds, NotifyOverlaySettings.MaxSeconds, 1, (s, v) => s with { Seconds = (float)v });
    ...
    private Slider NotifySlider(double minimum, double maximum, double step, Func<NotifyOverlaySettings, double, NotifyOverlaySettings> change)
    {
