...
/// <summary>
/// The groups using Modbot and the instances they have open, read from Modbot Cloud and held for a
/// short while.
/// </summary>
/// <remarks>
/// <para>
/// <strong>The key never leaves this server.</strong> The browser asks this site for
/// <c>/api/instances</c>; this site asks Cloud with <c>MODBOT_CLOUD_API_KEY</c> and hands back only
/// what came out. There is no other way the page could get the list without putting the key in
/// front of everyone who opens it.
/// </para>
/// <para>
/// One read is shared by every visitor for <see cref="Freshness"/>. A group's instances change on the
...
/// </para>
/// </remarks>
public sealed class OpenInstances(LandingEnvironment environment, IHttpClientFactory clients, ILogger<OpenInstances> log)
{
    ...
    public const string CloudPath = "/api/v1/public-instances";
    ...
    public async Task<string> ReadAsync(TimeProvider time, CancellationToken ct = default)
    {
        ...
        if (!environment.CanReadInstances)
            return Empty;
    ...
    private async Task<string?> FetchAsync(CancellationToken ct)
    {
        try
        {
            ...
            {
                log.LogInformation(
                    "Modbot Cloud did not give the open instances: {Status}", (int)response.StatusCode);
                return null;
            }
        ...
        {
            log.LogInformation("Could not read the open instances from Modbot Cloud: {Reason}", ex.Message);
            return null;
        }
    }
}
