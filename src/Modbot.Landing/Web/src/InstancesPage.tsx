...
/*
 * /instances — every group whose Modbot reports its open instances, and the instances they have open.
 *
 * The list is read from this site's own /api/instances, which reads Modbot Cloud with a key the browser
  ...
  return (
    <li className="flex min-w-0 flex-col overflow-hidden rounded-xl border bg-card">
      ...
      {group.instances.length === 0 ? (
        <p className="border-t px-4 py-3.5 text-sm text-muted-foreground">No instances open</p>
      ) : (
        <ul className="border-t">
          {group.instances.map((instance) => (
            <InstanceRow key={instance.location} instance={instance} now={now} />
          ))}
        </ul>
      )}
    </li>
...
function InstanceRow({ instance, now }: { instance: Instance; now: number }) {
  const region = regionName(instance.region)
  const age = openFor(instance.openedAt, now)

  return (
    <li className="flex min-w-0 items-center gap-3 px-4 py-3 [&:not(:last-child)]:border-b">
      {instance.worldImageUrl ? (
        <img
          src={instance.worldImageUrl}
          alt=""
          loading="lazy"
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-[0.9375rem]">{instance.worldName ?? instance.worldId}</p>
        <p className="truncate text-sm text-muted-foreground">{[region, age].filter(Boolean).join(' · ')}</p>
      </div>

      {instance.joinLink && (
        <a
          href={instance.joinLink}
          rel="noopener nofollow"
