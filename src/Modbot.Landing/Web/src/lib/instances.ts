/*
 * The shapes and the small pieces of arithmetic behind /instances, apart from the page so they can be
...
export function order(groups: Group[]): Group[] {
  return [...groups].sort((a, b) => {
    if ((a.instances.length > 0) !== (b.instances.length > 0)) return a.instances.length > 0 ? -1 : 1
