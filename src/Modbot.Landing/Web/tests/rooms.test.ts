...
import { type Group, openFor, order, regionName } from '../src/lib/rooms.ts'

const group = (groupId: string, groupName: string | null, rooms: number): Group => ({
  groupId,
  groupName,
...
test('how long a room has been open is the largest whole unit', () => {
...
test('groups with rooms come first, then by name, and a group with none is still listed', () => {
