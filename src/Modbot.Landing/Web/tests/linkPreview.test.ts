...
/**
 * The pages people share. The link preview a chat app draws comes from the head of the built file,
 * which is why these tags are written here rather than set by the app: a crawler runs no script.
 *
 * 404.html is left out on purpose. A page that says something is not there has nothing to preview,
 * and giving it the product's banner would make a dead link look like a working one.
 */
const pages = ['index.html', 'instances.html', 'privacy.html']
...
test('every page has a title, a description, an image and a site name', () => {
  for (const page of pages) {
    ...
    for (const name of ['og:type', 'og:site_name', 'og:url', 'og:title', 'og:description', 'og:image']) {
