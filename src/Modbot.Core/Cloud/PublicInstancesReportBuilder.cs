...
/// <summary>
/// Builds the public instances report out of what this Modbot already knows.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// </remarks>
public sealed class PublicInstancesReportBuilder(ModbotContext db, ModbotCloudAddress cloud)
{
    /// <summary>The one group access type that means "anyone can join".</summary>
    /// <remarks>
    /// VRChat writes <c>members</c> for an instance only the group can join and <c>plus</c> for members
    /// and their friends. Neither is public, and neither is ever reported: a link to one on a web
    /// page hands out an address the group deliberately kept inside. An instance whose access type
    ...
    /// </summary>
    public async Task<PublicInstancesReport?> BuildAsync(CancellationToken ct = default)
    {
        if (cloud.Disabled)
            return null;
        ...
        if (!settings.SharePublicInstances)
            return null;
        ...
        var instances = open
            .Select(r =>
            {
                ...
                return new PublicInstanceReport(
                    r.Location,
                    r.WorldId,
                    world?.Name,
        ...
        return new PublicInstancesReport(
            groupId,
