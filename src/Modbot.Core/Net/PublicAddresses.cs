...
/// <summary>
/// Which addresses an address somebody typed into Modbot may reach: public ones only.
/// </summary>
/// <remarks>
/// <para>
/// Moved here from the webhook guard (API keys design §6.7) when the calendar started fetching a
/// picture link for Discord event covers (calendar design §3.2). Both are the same risk -- a person
/// with a setting making Modbot's server call its own network -- and one list of ranges is easier to
/// keep right than two.
/// </para>
/// <para>
/// The check that matters is made when the connection is made, on the address the name actually
/// resolved to (<see cref="ConnectAsync"/>). A check at save time alone is beaten by a name that
/// resolves somewhere public on Monday and to <c>169.254.169.254</c> on Tuesday.
/// </para>
/// </remarks>
public static class PublicAddresses
{
    ...
    /// <summary>
    /// A <see cref="SocketsHttpHandler.ConnectCallback"/> that resolves the name, drops every blocked
    /// address, and connects to what is left.
    /// </summary>
    /// <param name="refuse">Makes the exception thrown when nothing is left.</param>
    public static async ValueTask<Stream> ConnectAsync(
        SocketsHttpConnectionContext context, Func<string, Exception> refuse, CancellationToken ct)
    {
