...
{
    public const string Tag = "Companion";
    ...
    /// <remarks>
    /// <para>
    /// <see cref="ICompanionDeviceStore"/> is <see cref="DatabaseCompanionDeviceStore"/>, backed by
    /// <c>companion_device</c> and <c>companion_pairing_code</c>. It replaced an in-memory store that
    ...
    public static IServiceCollection AddClientApi(this IServiceCollection services)
    {
        ...
        services.AddScoped<ICompanionDeviceStore, DatabaseCompanionDeviceStore>();
    ...
    public static IEndpointRouteBuilder MapClientApi(this IEndpointRouteBuilder app)
    {
        ...
        var companion = app.MapGroup("/api/v{apiVersion:int}/companion").WithTags(Tag).ExcludeFromDescription();
        ...
        companion.MapPost("/pair", PairHandler.HandleAsync)
            ...
            .Produces<CompanionError>(StatusCodes.Status503ServiceUnavailable)
            .AllowAnonymous();

        companion.MapGet("/time", (int apiVersion, IModbotClock clock) =>
                CompanionApiVersion.IsSupported(apiVersion)
        ...
        companion.MapPost("/events", EventsHandler.HandleAsync)
            ...
            .WithDescription(
                ...
                + "request.\n\n"
                + "Deduplication is a windowed range check across clients; the companionEventId "
            ...
            .Produces<CompanionError>(StatusCodes.Status409Conflict)
            .Produces<CompanionError>(StatusCodes.Status413PayloadTooLarge)
            .AllowAnonymous();
