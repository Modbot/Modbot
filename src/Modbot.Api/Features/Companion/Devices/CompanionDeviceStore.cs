...
/// steps.</para>
/// </remarks>
public interface ICompanionDeviceStore
{
...
/// implementation needs a <c>companion_device</c> table and a migration, which this change does not
/// add — see the notes on <see cref="ICompanionDeviceStore"/>.</para>
...
/// </remarks>
public sealed class InMemoryClientDeviceStore : ICompanionDeviceStore
{
    private readonly Dictionary<string, PairingCode> _codes = new(StringComparer.Ordinal);
    private readonly Dictionary<Guid, CompanionDevice> _devices = [];
    ...
    public Task<CompanionDevice> AddDeviceAsync(CompanionDevice device, CancellationToken ct)
    {
        lock (_gate)
    ...
    public Task<CompanionDevice?> FindByTokenHashAsync(string tokenHash, CancellationToken ct)
    {
        lock (_gate)
        {
    ...
    public Task<IReadOnlyList<CompanionDevice>> ListDevicesAsync(CancellationToken ct)
    {
        lock (_gate)
