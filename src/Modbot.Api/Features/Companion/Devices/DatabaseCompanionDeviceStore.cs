...
/// <summary>
/// The durable <see cref="ICompanionDeviceStore"/>, backed by <c>companion_device</c> and
/// <c>companion_pairing_code</c>.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// </remarks>
public sealed class DatabaseCompanionDeviceStore(ModbotContext db) : ICompanionDeviceStore
{
    public async Task<PairingCode> IssueCodeAsync(PairingCode code, CancellationToken ct)
    {
        ArgumentNullException.ThrowIfNull(code);

        db.CompanionPairingCodes.Add(new CompanionPairingCodeRecord
        {
    ...
    {
        var redeemed = await db.CompanionPairingCodes
        ...
        var row = await db.CompanionPairingCodes.AsNoTracking()
    ...
    public async Task<CompanionDevice> AddDeviceAsync(CompanionDevice device, CancellationToken ct)
    {
    ...
    /// </remarks>
    public async Task<CompanionDevice?> FindByTokenHashAsync(string tokenHash, CancellationToken ct)
    {
    ...
    {
        var changed = await db.CompanionDevices
    ...
    private static CompanionDevice Map(CompanionDeviceRecord r) => new(
        r.Id,
        r.TokenHash,
        r.CompanionVersion,
        r.Platform,
