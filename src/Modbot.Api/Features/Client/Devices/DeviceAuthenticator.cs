...
public readonly record struct DeviceAuthentication(ClientDevice? Device, IResult? Failure)
{
...
{
    private readonly IClientDeviceStore _devices;
    private readonly IModbotClock _clock;

    public DeviceAuthenticator(IClientDeviceStore devices, IModbotClock clock)
    {
        _devices = devices;
        _clock = clock;
    }
    ...
    public async Task<DeviceAuthentication> AuthenticateAsync(HttpContext context, CancellationToken ct)
    {
        ...
        if (ReadBearer(context) is not { Length: > 0 } token)
            return new DeviceAuthentication(null, ClientApiErrors.Unauthorised());
        ...
        // stale log parser.
        await _devices.TouchAsync(device.Id, _clock.UtcNow, ReadClientVersion(context) ?? device.ClientVersion, ct);
    ...
    private static string? ReadClientVersion(HttpContext context)
        => context.Request.Headers.TryGetValue("X-Modbot-Client-Version", out var values)
...
/// </remarks>
public static class ClientApiVersion
{
