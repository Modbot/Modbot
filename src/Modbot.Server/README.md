...
| `MODBOT_CLOUD_ENDPOINT` | No | `https://cloud.modbot.co` | The Modbot Cloud this server talks to for its own use: the term lists, a report every six hours, the rooms its group has open, and its own log. A value that is not a full `http` or `https` address means the default. Companions are not told it. |
