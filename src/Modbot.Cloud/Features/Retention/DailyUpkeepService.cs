    ...
    ILogger<DailyUpkeepService> log) : BackgroundService
{
    ...
    internal async Task<bool> RunOnceAsync(CancellationToken ct)
    {
        try
        {
            ...
            // The months server_log writes into, made ahead of Cloud's clock, before the months
            ...
            await scope.ServiceProvider.GetRequiredService<ServerLogs.LogPartitionMaintainer>().EnsureAsync(ct);

            var months = await scope.ServiceProvider.GetRequiredService<ServerLogs.LogRetention>().RunAsync(ct);
            if (months.Count > 0)
